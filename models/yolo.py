    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            try:  #train
                ...
                x[i] = self.m[i](quantize_tg(x[i], quantizer=self.quant))  # conv
            ...
            if not self.training:  # inference
                ...
                if isinstance(self, Segment):  # (boxes + masks)
                    ...
                    xy = quant_mul(quant_add(quant_mul(xy,2) ,self.grid[i],quantizer=self.quant) , self.stride[i], self.quant, self.dequant)  # xy
                    wh = quant_mul(quant_pow((quant_mul(wh,2)), 2, quantizer=self.quant, dequantizer=self.dequant), self.anchor_grid[i], self.quant, self.dequant)  # wh
                    conf = quantize_tg(conf, quantizer=self.quant)  # conf quantize
...
class DetectionModel(BaseModel):
    ...
    def forward(self, x, augment=False, profile=False, visualize=False):
        ...
        if augment:
            ...
            x = recursive_apply(x, force_dequantize)
            return x
