...
def run(
    ...
    # Quantization
    '''
    ################################# QUANTIZATION MOD BY TUNGDD #############################
    first_time_quantize:
        True: the model is quantized for the first time and save to a .pt file , also run val on the quantized model
        False: if you already have the quantized weight. If you set this to False, you need to set ready to True
        MUST set False when training the model
    
    ready:
        True: you already have the quantized weight, and you want to run val on it
        False: you don't want to load the quantized weight
    
    '''
    first_time_quantize = True
    ready = True

    if ready:
        first_time_quantize = False
    if first_time_quantize:
        checkpoint = torch.load(weights[0], map_location=device)  # load checkpoint
        model.model.eval()
        ...
        quantized_ckpt = {k: v for k, v in checkpoint.items() if k != 'model'}
        quantized_ckpt['model'] = model_int8
        model.model = model_int8
        torch.save(model.model.state_dict(), "quantized_model_state.pth")
    if ready:
